...
import { FileMap } from '../../../models/file';
...
export const FileTree = memo(
  ({
    ...
    useEffect(() => {
        // console.log('collapsed :', collapsed, fileList.length, fileList);
      if (collapsed) {
    ...
    return (
      <div className={`${className || ''} ${s.fileTree}`}>
            ...
            case 'file': {
              return (
                <File
                  ...
                  unsavedChanges={fileOrFolder.modified}
                  onClick={() => {
...
function Folder({ folder: { depth, name }, collapsed, selected = false, onClick }: FolderProps) {
  return (
    ...
    //   className=''
      depth={depth}
      iconClasses={collapsed ? 'icon-right' : 'icon-down'}
...
function File({ file: { depth, name }, onClick, selected, unsavedChanges = false }: FileProps) {
  return (
    <NodeButton
      className={selected ? s.selectedFile : ''}
      depth={depth}
      iconClasses={'icon-file'}
      onClick={onClick}
    >
      <div
      ...
      >
        ...
        {unsavedChanges && <div className={s.unsaveIcon}></div>}
...
function NodeButton({ depth, iconClasses, onClick, className, children }: ButtonProps) {
  return (
    <button
      ...
      onClick={() => onClick?.()}
    >
      <div className={`icon ${iconClasses}`} style={{marginRight: '6px'}}></div>
      <div className={s.btnContent}>{children}</div>
    </button> 
  );
}
...
//   console.log('nodeList :', nodeList);
  ...
  // console.log('node :', nodeList, nodeMap, childrenMap);

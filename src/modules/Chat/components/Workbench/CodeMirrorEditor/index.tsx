...
import { Theme, EditorSettings, EditorDocument,  TextEditorDocument, ScrollPosition} from '../types/index';
  ...
  ({
    ...
    return (
      <div className={`${s.editor} ${className}`}>
        ...
        <div className={s.editorWrap} ref={containerRef} />
      </div>
    );
  },
);
    ...
    extensions: [
      ...
      foldGutter({
        markerDOM: (open) => {
          ...
          icon.className = `icon icon-small ${open ? 'icon-down' : 'icon-right'} pointer`
    ...
    .map((range) => {
      return {
        ...
        create: () => {
          ...
          divElement.textContent = 'AI正在生成中，文件无法修改';
